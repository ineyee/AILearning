...
* **自动部署模型**：安装 Ollama 后，只需要在终端执行 `ollama run 模型名称` 命令，就会自动下载和安装指定的模型到本机
* **本地运行模型**： `ollama run 模型名称` 命令用来运行模型，这样一来我们就可以不调用云端 API，而是在本机上聊天、写代码、总结文本、翻译、做问答等，适合需要离线或隐私要求高的场景，长期使用的成本也可能低于频繁调用云端 API
* **给应用提供本地 AI 接口**：Ollama 自带 REST API，默认可以通过类似 `http://localhost:11434/api/chat` 的接口调用模型，所以开发者可以把它接到自己的脚本、网站、应用、RAG 知识库或自动化工具里
