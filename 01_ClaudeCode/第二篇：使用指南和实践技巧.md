...
* 没什么固定模板，新项目时可以执行 `/init` 命令让 Claude Code 根据不同的项目自己生成合适的 CLAUDE.md 文件，我们再在此基础上改改
* 后续当项目文件发生较大变化时，再次执行 `/init` 命令让 Claude Code 更新下 CLAUDE.md 文件
...
* 按照 plugin.json 文件里声明的路径，把相应的“能力”放进 plugin 里，my-first-plugin 就算编写完成了
...
* 但是第一篇里我们说过**我们不能直接给 CC 安装 plugin，必须先把 plugin 上传到插件市场，然后再经由插件市场给 CC 安装 plugin。并且我们又只能从别人的插件市场里安装插件，而不能往别人的插件市场里上传插件。所以如果我们想共享或开源我们的 plugin，就必须自己维护一个插件市场，我们只能往自己维护的插件市场里上传插件。**现在创建我们自己的插件市场，随便找个地方创建一个文件夹，这个文件夹的名字必须得是 my-marketplace，它就是我们的插件市场
