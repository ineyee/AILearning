...
```markdown
//--- Subagents 模板 ---//
// Subagents 的本质不是一个“命令”，而是一个“执行角色”
// 我们可以通过 @subagent-name 这个强制路由标识来直接呼叫某个 subagent 来执行任务，除此之外模型还可以通过用户提示词的意图来自动触发某个 subagent

// 元数据部分：name 和 description 必填，其它 4 项可选
---
name: xxx         // subagent 的名称，见名知意，最好字母、数字、中划线
description: xxx  // subagent 的角色、目标和触发条件，非常关键，触发条件要表明”意图触发“而不是”关键词触发“
model: inherit    // subagent 要用哪个模型，默认 inherit — 继承主 agent 的模型，也可以指定 opus、sonnet、haiku
tools: ALL        // subagent 可用的工具（根据实际场景从 CC 内置工具里选），默认值 ALL
color: red        // 给 subagent 配个颜色以便分辨，red orange yellow green cyan blue purple pink，默认值 Automatic color
---
