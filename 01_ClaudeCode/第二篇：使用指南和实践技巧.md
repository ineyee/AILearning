...
不同的作用域主要体现在 `~/.claude/plugins/installed_plugins.json` 文件里，Claude Code 启动时会根据 installed_plugins.json 文件里的记录判断当前项目该不该激活某个 plugin
...
接下来我们自己编写一个 my-purchase-plugin 用用，它把合并报价表 subagent 和计算售价 subagent 打包在一起，方便团队成员一键安装

###### 3.1 编写

* 随便找个地方创建一个 my-purchase-plugin 目录，这个目录就是我们的 plugin

* 在这个目录里创建 plugin.json 文件，这是 plugin 的清单文件，描述了 plugin 的基本信息和包含的组件
...
* 按照 plugin.json 里声明的路径，创建对应的组件文件，merge-quotes.md 和 calc-price.md 的内容就是我们在 Subagents 章节里编写的那两个 subagent，这里不再重复
...
* plugin 里除了可以打包 Subagents，还可以打包 MCP Servers、Skills、Hooks，只需要在 plugin.json 的 components 里声明，并在对应目录里放好文件即可

  ```json
  {
    "name": "my-purchase-plugin",
    "version": "1.0.0",
    "description": "采购报价表处理工具",
    "author": "你的名字",
    "components": {
      "mcpServers": ["mcp-servers/..."],
      "skills": ["skills/..."],
      "agents": ["agents/merge-quotes.md", "agents/calc-price.md"],
      "hooks": ["hooks/..."]
    }
  }
  ```
...
* 以上我们的 my-purchase-plugin 就算编写完成了。plugin install 只支持从 marketplace 安装，不支持直接指定本地路径，所以本地 plugin 需要先把所有 plugin 的共同父目录注册为本地插件市场，再安装

* 在所有 plugin 的共同父目录（我们叫它 pluginplaces）下创建 .claude-plugin 目录，在 .claude-plugin 目录下创建 marketplace.json 文件

  ```
  pluginplaces/                    # 所有 plugin 的共同父目录
  ├── .claude-plugin/
  │   └── marketplace.json         # 本地插件市场清单
  └── my-purchase-plugin/          # 我们的 plugin
      ├── plugin.json
      └── agents/
          ├── merge-quotes.md
          └── calc-price.md
  ...
  ```json
  // pluginplaces/.claude-plugin/marketplace.json
