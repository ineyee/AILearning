...
* 但是第一篇里我们说过**我们不能直接给 CC 安装 plugin，必须先把 plugin 上传到插件市场，然后再经由插件市场给 CC 安装 plugin。并且我们又只能从别人的插件市场里安装插件，而不能往别人的插件市场里上传插件。所以如果我们想共享或开源我们的 plugin，就必须自己维护一个插件市场，我们只能往自己维护的插件市场里上传插件。**现在创建我们自己的插件市场，随便找个地方创建一个文件夹，my-marketplace，它就是我们的插件市场

* 然后在 my-marketplace 文件夹里创建一个 .claude-plugin 文件夹，在 .claude-plugin 文件夹里创建一个 marketplace.json 文件，插件市场的基本信息和包含的“能力”就写在这个文件里
...
* 给 Claude Code 添加我们自己的本地插件市场，插件市场会统一全局安装在 `~/.claude/plugins/marketplaces` 目录下
...
* 重启 Claude Code，输入 `/plugins` 命令应该就能看到我们刚安装的插件市场了
  ...
  ![image-20260506135155095](img/image-20260506135155095.png)
...
* 执行 shell 命令安装本地插件市场里的插件，注意作用域，这里选择安装到项目级
  ...
  # 安装到本地级
  claude plugin install my-first-plugin --scope local
  
  # 安装到项目级
  claude plugin install my-first-plugin --scope project
  
  # 安装到用户级
  claude plugin install my-first-plugin
  claude plugin install my-first-plugin --scope user
  ```
...
* 重启 Claude Code，输入 `/plugins` 命令应该就能看到我们刚安装的 plugin 了
  ...
  ![image-20260506135802937](img/image-20260506135802937.png)

  ![image-20260506135915223](img/image-20260506135915223.png)

  ![image-20260506140033058](img/image-20260506140033058.png)

  ![image-20260506152051148](img/image-20260506152051148.png)
