...
> **所以 CLAUDE.md 里就可以只写一些项目级的整体背景了、就不用再写所有任务的具体规则了**

* 项目类型太多、不太好整固定模板，新项目时可以执行 `/init` 命令让 Claude Code 自己生成合适的 CLAUDE.md 文件，我们在此基础上改改

* 后续当项目文件发生较大变化时，再次执行 `/init` 命令让 Claude Code 及时更新下 CLAUDE.md 文件，不要让这个文件落后腐化
* 建议把 CLAUDE.md 文件提交到 Git 与团队共享
* 建议把 CLAUDE.md 文件搞成个一百行左右的目录索引 + 引用其它文件，这有助于组织结构，但不会减少上下文占用、还是一次性全部加载所有文件

![image-20260507091535031](img/image-20260507091535031.png)
...
不过需要注意的是 plugin 被安装后并不是把各“能力”分散地存储到各“能力”对应的存储位置，而是始终作为一个整体存储在全局 `~/.claude/plugins/cache`
...
```json
{
  ...
  "plugins": {
    ...
    // my-first-plugin 这个 plugin
    "my-first-plugin@my-marketplace": [
      {
        // 对于 plugin_demo 这个项目来说
        "projectPath": "/Users/yiyi/Desktop/AILearning/01_ClaudeCode/plugin_demo",
        // 该 plugin 是项目级作用域
        "scope": "project",
        // plugin 本身的安装位置
        "installPath": "/Users/yiyi/.claude/plugins/cache/my-marketplace/my-first-plugin/1.0.0"
      }
    ]
  }
}
...
```shell
# 命令格式
claude plugin install <plugin-name>@<marketplace-name> [options]
...
<img src="img/image-20260506221355864.png" alt="image-20260506221355864" style="zoom:50%;" />
...
* 然后在 my-first-plugin 文件夹里创建一个 .claude-plugin 文件夹，在 .claude-plugin 文件夹里创建一个 plugin.json 文件，plugin 的基本信息和包含的“能力”就写在这个文件里
...
* 以上我们的 my-first-plugin 就算编写完成了，但是第一篇里我们说过**我们不能直接给 CC 安装 plugin，必须先把 plugin 上传到插件市场，然后再经由插件市场给 CC 安装 plugin。并且我们又只能从别人的插件市场里安装插件，而不能往别人的插件市场里上传插件。所以如果我们想共享或开源我们的 plugin，就必须自己维护一个插件市场，我们只能往自己维护的插件市场里上传插件。**现在创建我们自己的插件市场，随便找个地方创建一个文件夹，这个文件夹的名字必须得是 my-marketplace，它就是我们的插件市场

* **【本地插件市场】**然后在 my-marketplace 文件夹里创建一个 .claude-plugin 文件夹，在 .claude-plugin 文件夹里创建一个 marketplace.json 文件，插件市场的基本信息和包含的“能力”就写在这个文件里
...
* 然后直接把 my-first-plugin 这个 plugin 目录复制进插件市场的根目录里即可，本地插件市场就算编写完成了
...
* **【本地插件市场】**给 Claude Code 添加我们自己的本地插件市场，插件市场会统一全局安装在 `~/.claude/plugins/marketplaces` 目录下

  ```shell
  # 命令格式
  claude plugin marketplace add ${本地插件市场的绝对路径或相对路径}
  
  # 安装命令
  claude plugin marketplace add ../my-marketplace
  ```

* **【远程插件市场】**给 Claude Code 添加我们自己的远程插件市场，插件市场会统一全局安装在 `~/.claude/plugins/marketplaces` 目录下

  ```shell
  # HTTPS 命令格式
  claude plugin marketplace add ${仓库完整地址里的 ownername/reponame}
  # HTTPS 安装命令
  claude plugin marketplace add ineyee/my-marketplace
  
  # SSH 命令格式
  claude plugin marketplace add ${仓库的完整地址}
  # SSH 安装命令
  claude plugin marketplace add git@github.com:ineyee/my-marketplace.git
  ```

* 重启 Claude Code，输入 `/plugins` 命令应该就能看到我们刚安装的插件市场了

  | 本地插件市场                                                | 远程插件市场                                                |
  | ----------------------------------------------------------- | ----------------------------------------------------------- |
  | ![image-20260506135155095](img/image-20260506135155095.png) | ![image-20260506160242096](img/image-20260506160242096.png) |

* 执行 shell 命令安装本地插件市场里的插件，注意作用域，这里选择安装到项目级

  ```shell
  # 安装到项目级
  claude plugin install my-first-plugin@my-marketplace --scope project
  ```

* 重启 Claude Code，输入 `/plugins` 命令应该就能看到我们刚安装的 plugin 了

  ![image-20260506135703479](img/image-20260506135703479.png)

  | mcp servers                                                 | skill                                                       |
  | ----------------------------------------------------------- | ----------------------------------------------------------- |
  | ![image-20260506135802937](img/image-20260506135802937.png) | ![image-20260506135915223](img/image-20260506135915223.png) |
  | **subagent**                                                | **hook**                                                    |
  | ![image-20260506140033058](img/image-20260506140033058.png) | ![image-20260506152051148](img/image-20260506152051148.png) |
...
后续如果在我们的插件市场里更新了旧插件 my-first-plugin 或者增加了新插件 my-second-plugin，用户可以：
...
# 然后更新旧插件
claude plugin update my-first-plugin@my-marketplace

# 或者安装新插件
claude plugin install my-second-plugin@my-marketplace --scope project
```